  ...
  return (
    <aside className="hidden lg:block w-64 shrink-0">
      <div className="sticky top-20 space-y-3">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-3.5 shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <select
            ...
            className={`w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-lg border border-gray-200 dark:border-gray-600 focus:border-gray-900 dark:focus:border-gray-400 outline-none transition-colors text-sm ${FOCUS_RING}`}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-3.5 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{market}</span>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-2.5">Click to analyze instantly</p>

          <div className="space-y-1 max-h-[500px] overflow-y-auto custom-scrollbar pr-0.5">
            {stocks.map((s) => {
              ...
              return (
                <button
                  ...
                  disabled={loading}
                  aria-pressed={active}
                  className={`w-full text-left px-3 py-2 rounded-lg border transition-all disabled:opacity-40 ${FOCUS_RING} ${
                    active
  ...
  return (
    <div className="lg:hidden mb-6">
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <button
          onClick={() => setOpen((o) => !o)}
          className={`w-full flex items-center justify-between px-4 py-3 text-sm font-semibold text-gray-800 dark:text-gray-200 ${FOCUS_RING}`}
          aria-expanded={open}
        >
            ...
            <div className="flex gap-1.5 mb-3 flex-wrap">
              {MARKETS.map((m) => (
                <button
                  key={m}
                  onClick={() => setMarket(m)}
                  className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${FOCUS_RING} ${
                    market === m
            ...
            <div className="grid grid-cols-2 gap-2">
              {stocks.map((s) => (
                <button
                  ...
                  disabled={loading}
                  className={`text-left p-3 rounded-lg border transition-all disabled:opacity-40 ${FOCUS_RING} ${
